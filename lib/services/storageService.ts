...
export async function getStorageAnalytics(groupId: string): Promise<StorageAnalytics> {
  await connectDB();
  ...
  const [totalMedia, processedMedia, largestFiles, oldestFiles, duplicates] =
    await Promise.all([
      ...
      findDuplicateCandidates(groupId),
    ]);
...
export async function performCleanup(
  ...
  if (options.deleteDuplicates) {
    const duplicates = await findDuplicateCandidates(groupId);
  ...
  mediaToDelete = await Media.find(query).select("_id s3Key s3Bucket fileSize");
  ...
  const s3Keys = mediaToDelete.map((item) => item.s3Key);
  const s3Bucket = mediaToDelete[0]?.s3Bucket; // All items in same group use same bucket
  ...
  // Delete from S3
  if (s3Keys.length > 0 && s3Bucket) {
    await bulkDelete(s3Keys, s3Bucket);
  }
...
export async function scheduleAutoCleanup(groupId: string): Promise<void> {
  await connectDB();
